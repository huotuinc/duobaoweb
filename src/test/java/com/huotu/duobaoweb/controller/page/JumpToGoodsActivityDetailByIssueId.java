 ...
 * 商品详情页面元素检查
 */
public class JumpToGoodsActivityDetailByIssueId {
    //活动已经开始，但还没有任何人参与的页面显示
    public void NoJoined(WebDriver driver, Goods goods, Issue issue) {
        driver.get("http://localhost:8080/goods/detailByGoodsId?id=" + issue.getId());
    ...
    public void WrongGoodsID(WebDriver driver, String issueId) {
        driver.get("http://localhost:8080/goods/detailByGoodsId?id=" + issueId);
        Assert.assertEquals("错误页面", driver.getTitle());
        Assert.assertEquals("参数错误", driver.findElement(By.xpath("XXXXX")));
    }

    public void NotFindGoodsID(WebDriver driver, Long issueId) {
        driver.get("http://localhost:8080/goods/detailByGoodsId?id=" + issueId);
        Assert.assertEquals("错误页面", driver.getTitle());
        Assert.assertEquals("商品不存在", driver.findElement(By.xpath("XXXXX")));
    }

    public void GoodsUnCheck(WebDriver driver, Long issueId) {
        driver.get("http://localhost:8080/goods/detailByGoodsId?id=" + issueId);
        Assert.assertEquals("错误页面", driver.getTitle());
        Assert.assertEquals("商品不存在", driver.findElement(By.xpath("XXXXX")));
    }

    public void GoodsDown(WebDriver driver, Long issueId) {
        driver.get("http://localhost:8080/goods/detailByGoodsId?id=" + issueId);
        Assert.assertEquals("错误页面", driver.getTitle());
        Assert.assertEquals("商品已下架", driver.findElement(By.xpath("XXXXX")));
    }

    public void Expired(WebDriver driver, Long issueId) {
        driver.get("http://localhost:8080/goods/detailByGoodsId?id=" + issueId);
        Assert.assertEquals("错误页面", driver.getTitle());
        Assert.assertEquals("商品活动已结束", driver.findElement(By.xpath("XXXXX")));
    }

    public void NotStart(WebDriver driver, Long issueId) {
        driver.get("http://localhost:8080/goods/detailByGoodsId?id=" + issueId);
        Assert.assertEquals("错误页面", driver.getTitle());
