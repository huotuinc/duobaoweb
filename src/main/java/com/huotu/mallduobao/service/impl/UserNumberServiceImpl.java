...
public class UserNumberServiceImpl implements UserNumberService {
    ...
    @Override
    public List<UserNumber> getLotteryBeforeTop20(Long date) {
        log.info("getLotteryBeforeTop20 enter");
        StringBuilder hql = new StringBuilder();
        hql.append("select u from UserNumber u where u.time<?1 order by u.time desc");
